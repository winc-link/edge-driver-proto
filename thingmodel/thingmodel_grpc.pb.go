...
func (UnimplementedThingModelUpServiceServer) ThingModelMsgReport(context.Context, *ThingModelMsg) (*drivercommon.CommonResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method ThingModelMsgReport not implemented")
}
...
func RegisterThingModelUpServiceServer(s grpc.ServiceRegistrar, srv ThingModelUpServiceServer) {
	// If the following call panics, it indicates UnimplementedThingModelUpServiceServer was
...
func (UnimplementedThingModelDownServiceServer) ThingModelMsgIssue(context.Context, *ThingModelIssueMsg) (*emptypb.Empty, error) {
	return nil, status.Error(codes.Unimplemented, "method ThingModelMsgIssue not implemented")
}
...
func RegisterThingModelDownServiceServer(s grpc.ServiceRegistrar, srv ThingModelDownServiceServer) {
	// If the following call panics, it indicates UnimplementedThingModelDownServiceServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
...
const (
	MessageRateService_MessageRate_FullMethodName = "/thingmodel.MessageRateService/MessageRate"
)

// MessageRateServiceClient is the client API for MessageRateService service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type MessageRateServiceClient interface {
	MessageRate(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (*MessageRateResponse, error)
}

type messageRateServiceClient struct {
	cc grpc.ClientConnInterface
}

func NewMessageRateServiceClient(cc grpc.ClientConnInterface) MessageRateServiceClient {
	return &messageRateServiceClient{cc}
}

func (c *messageRateServiceClient) MessageRate(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (*MessageRateResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(MessageRateResponse)
	err := c.cc.Invoke(ctx, MessageRateService_MessageRate_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// MessageRateServiceServer is the server API for MessageRateService service.
// All implementations must embed UnimplementedMessageRateServiceServer
// for forward compatibility.
type MessageRateServiceServer interface {
	MessageRate(context.Context, *emptypb.Empty) (*MessageRateResponse, error)
	mustEmbedUnimplementedMessageRateServiceServer()
}

// UnimplementedMessageRateServiceServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedMessageRateServiceServer struct{}

func (UnimplementedMessageRateServiceServer) MessageRate(context.Context, *emptypb.Empty) (*MessageRateResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method MessageRate not implemented")
}
func (UnimplementedMessageRateServiceServer) mustEmbedUnimplementedMessageRateServiceServer() {}
func (UnimplementedMessageRateServiceServer) testEmbeddedByValue()                            {}

// UnsafeMessageRateServiceServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to MessageRateServiceServer will
// result in compilation errors.
type UnsafeMessageRateServiceServer interface {
	mustEmbedUnimplementedMessageRateServiceServer()
}

func RegisterMessageRateServiceServer(s grpc.ServiceRegistrar, srv MessageRateServiceServer) {
	// If the following call panics, it indicates UnimplementedMessageRateServiceServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
	...
	s.RegisterService(&MessageRateService_ServiceDesc, srv)
}

func _MessageRateService_MessageRate_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(emptypb.Empty)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(MessageRateServiceServer).MessageRate(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: MessageRateService_MessageRate_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(MessageRateServiceServer).MessageRate(ctx, req.(*emptypb.Empty))
	}
	return interceptor(ctx, in, info, handler)
}

// MessageRateService_ServiceDesc is the grpc.ServiceDesc for MessageRateService service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var MessageRateService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "thingmodel.MessageRateService",
	HandlerType: (*MessageRateServiceServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "MessageRate",
			Handler:    _MessageRateService_MessageRate_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "thingmodel/thingmodel.proto",
